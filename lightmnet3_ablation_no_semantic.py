...
class LightMNet(nn.Module):
    """
    Ablation variant without the semantic cross-fusion module.
    Metric cues are still injected into the aggregated backbone feature.
    """
    ...
    @staticmethod
    def aggregate_diff(diff_feats, output_size):
        resized = []

        for diff in diff_feats:
            if diff.shape[2:] != output_size:
                diff = F.interpolate(
                    diff,
                    size=output_size,
                    mode="bilinear",
    ...
    def forward(self, img_t1, img_t2):
        ...
        diff_feats = self.metric_enhance(feat_t1, feat_t2)
        ...
        diff_context = self.aggregate_diff(diff_feats, (base_h, base_w))
        fused = base_agg * (1.0 + diff_context)
