...
def load_model(pretrained=True):
    ...
    return LightMNet(pretrained=pretrained)


def apply_freeze_strategy(model, freeze_mode):
    for param in model.parameters():
        param.requires_grad = False

    if freeze_mode == "freeze_layer0_1":
        ...
        for param in model.layer4.parameters():
            param.requires_grad = True
    elif freeze_mode == "freeze_layer0_2":
        for param in model.layer3.parameters():
            param.requires_grad = True
        for param in model.layer4.parameters():
            param.requires_grad = True
    elif freeze_mode == "freeze_layer0_3":
        for param in model.layer4.parameters():
            param.requires_grad = True
    else:
...
def run_training(args):
    ...
    print(f"lr={args.lr}, weight_decay={args.weight_decay}, "
          f"pos_weight={args.pos_weight}, batch_size={args.batch_size}", flush=True)
    print(f"freeze_mode={args.freeze_mode}, optimizer={args.optimizer}", flush=True)
    ...
    model = load_model(pretrained=args.pretrained_backbone).to(device)
    ...
    apply_freeze_strategy(model, args.freeze_mode)
...
def build_parser():
    ...
    parser.add_argument("--target_f1", type=float, default=1.0)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--freeze_mode", type=str, default="freeze_layer0_2",
                        choices=["freeze_layer0_1", "freeze_layer0_2", "freeze_layer0_3"])
